...
function slideTitle({ lineTop, highlightWord, lineBottom, badge, body }) {
  return page(`
    ...
    <div style="position:absolute;top:300px;left:64px;width:940px;">
      <div style="margin-bottom:18px;">${highlight(highlightWord, { color: COLORS.green, rotate: -4, size: 76 })}</div>
      <h1 style="font-size:76px;color:${COLORS.ink};">${lineBottom}</h1>
      <div style="margin-top:26px;">${`<span class="pill" style="font-size:52px;">${badge}</span>`}</div>
      <p style="margin-top:44px;font-size:26px;font-weight:600;max-width:600px;line-height:1.5;">${body}</p>
    </div>
...
function slideBrowserQuote({ headlinePlain, headlineHighlight, quote, subtext }) {
  return page(`
    ...
    <div style="position:absolute;top:190px;left:0;width:1080px;text-align:center;">
    ...
    <div class="browser-card" style="position:absolute;top:420px;left:70px;width:940px;">
      ...
      <div class="body" style="text-align:center;">
        ...
        <p style="font-family:'Baloo 2';font-weight:700;font-size:36px;line-height:1.25;margin-top:10px;">${quote}</p>
        <p style="font-size:22px;font-weight:500;margin-top:22px;line-height:1.5;color:#3a382f;">${subtext}</p>
      </div>
    </div>
...
function slideChecklist({ headlinePlain, headlineHighlight, items, body }) {
  ...
  const itemsHtml = items.map((t, i) => `
    <div class="checklist-item" style="margin-bottom:34px;">
      <span class="bullet" style="background:${dotColors[i % 3]};">${check({ size: 26, color: '#fff' })}</span>
      <span style="font-family:'Baloo 2';font-weight:700;font-size:36px;">${t}</span>
    </div>`).join('');
  ...
  return page(`
    ...
    <div style="position:absolute;top:180px;left:0;width:1080px;text-align:center;">
      <h1 style="font-size:66px;color:${COLORS.ink};">${headlinePlain}</h1>
      <div style="margin-top:18px;">${highlight(headlineHighlight, { color: COLORS.orange, rotate: -3, size: 50, filled: true })}</div>
    </div>

    <div style="position:absolute;top:420px;left:150px;width:780px;">
      ${itemsHtml}
    </div>

    <div style="position:absolute;top:700px;left:0;width:1080px;text-align:center;display:flex;justify-content:center;gap:16px;">
    ...
    <div style="position:absolute;top:780px;left:0;width:1080px;text-align:center;">
      <p style="font-size:26px;font-weight:600;line-height:1.6;">${nl2br(body)}</p>
    </div>
...
function slideBrowserStatement({ headlinePlain, headlineHighlight, statement, subtextTop, subtextBottom }) {
  return page(`
    ...
    <div style="position:absolute;top:110px;left:0;width:1080px;text-align:center;">
      <h1 style="font-size:66px;color:${COLORS.ink};">${headlinePlain}</h1>
    ...
    <div class="browser-card" style="position:absolute;top:380px;left:80px;width:920px;">
      ...
      <div class="body" style="text-align:center;">
        <p style="font-family:'Baloo 2';font-weight:700;font-size:34px;line-height:1.3;">${statement}</p>
    ...
    <div style="position:absolute;top:850px;left:0;width:1080px;text-align:center;">
      <p style="font-size:26px;font-weight:600;">${subtextTop}</p>
      <p style="font-size:26px;font-weight:700;margin-top:6px;">${subtextBottom}</p>
    </div>
...
function slideCTA({ lineTop, badge, lineMid, highlightWord, body, ctaLine, ctaSub }) {
  return page(`
    ...
    <div style="position:absolute;top:150px;left:0;width:1080px;text-align:center;">
      ...
      <p style="margin-top:36px;font-size:26px;font-weight:600;max-width:680px;margin-left:auto;margin-right:auto;line-height:1.5;">${body}</p>
    ...
    <div style="position:absolute;top:1010px;left:120px;width:640px;">
      <p style="font-size:30px;font-weight:700;">${ctaLine}</p>
      <p style="font-size:24px;font-weight:500;margin-top:6px;line-height:1.5;">${ctaSub}</p>
    </div>
